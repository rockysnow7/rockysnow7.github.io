...
Let \\( \text{AE}_{i,j,k,...} : \mathbb R^n \rightarrow \mathbb R^{m_i,j,k,...} \\) be
the autoencoder associated with the cluster centre \\( i \\), sub-cluster centre \\( j
\\), sub-sub-cluster centre \\( k \\), etc., where \\( i \in \mathbb R^n \\).
